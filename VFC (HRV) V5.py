...
sujet_nom_prompt.grid(row = row_var, column = 0, columnspan = 2, padx = 10, pady = 10)
...
sujet_nom_boutton.grid(row = row_var, column = 2, columnspan=2, padx = 10, pady = 10)
...
numero_part_prompt.grid(row = row_var, column = 0, columnspan = 2, padx = 10, pady = 10)
...
numero_part_boutton.grid(row = row_var, column = 2, columnspan=2, padx = 10, pady = 10)

########################################################################################NOM DU FICHIER########################################################################################

def on_entry_click_nom_pdf(event):
    if nom_pdf_prompt.get() == "Nom du fichier (sans l'extension .pdf)" :
        nom_pdf_prompt.delete(0, "end")  # Delete the current content of the Entry widget
        nom_pdf_prompt.insert(0, '')  # Insert an empty string

def on_focus_out_nom_pdf(event):
    if nom_pdf_prompt.get() == '' :
        nom_pdf_prompt.insert(0, "Nom du fichier (sans l'extension .pdf)")  # If the Entry widget is empty, insert the default text

def Confirmer_nom_pdf():
    global nom_pdf
    nom_pdf = f'{nom_pdf_prompt.get()}.pdf'
    print(nom_pdf)

row_var += 1

nom_pdf_prompt = ttk.Entry(window1)
nom_pdf_prompt.insert(0, "Nom du fichier (sans l'extension .pdf)")
nom_pdf_prompt.grid(row = row_var, column = 0, columnspan=2, padx = 10, pady = 10)
nom_pdf_prompt.bind("<FocusIn>", on_entry_click_nom_pdf)  # Bind the on_entry_click function to the FocusIn event
nom_pdf_prompt.bind("<FocusOut>", on_focus_out_nom_pdf)  # Bind the on_focus_out function to the FocusOut event

nom_pdf_boutton = ttk.Button(window1, text = 'Confirmer', command=Confirmer_nom_pdf)
nom_pdf_boutton.grid(row = row_var, column = 2, columnspan=2, padx = 10, pady = 10)
...
nombre_de_pages_prompt.grid(row = row_var, column = 0, columnspan = 2, padx = 10, pady = 10)
...
nombre_de_pages_boutton.grid(row = row_var, column = 2, columnspan=2, padx = 10, pady = 10)
...
hauteur_prompt.grid(row = row_var, column = 0, columnspan = 2, padx = 10, pady = 10)
...
hauteur_boutton.grid(row = row_var, column = 2, columnspan=2, padx = 10, pady = 10)
...
nbs_prompt.grid(row = row_var, column = 0,  columnspan = 2, padx = 10, pady = 10)
...
nbs_boutton.grid(row = row_var, column = 2, columnspan=2, padx = 10, pady = 10)
...
hauteur_label.grid(row= row_var, column=0, columnspan = 3, pady = 10)
row_var += 1
confirm_label = ttk.Label(window1, text = 'Appuyez sur confirmer avant de fermer cette page.')
confirm_label.grid(row= row_var, column=0, columnspan = 3, pady = 10)


window1.mainloop()

nbs = int(nbs)
hauteur = 1290 - int(hauteurp)
...
if nombre_de_pages == 'Toutes' or nombre_de_pages == 'All' or nombre_de_pages == 'toutes' or nombre_de_pages == 'all' :
        derniere_page = len(doc_PDF)+1
        premiere_page = 1
        nombre_de_pages = range(premiere_page, derniere_page)
elif '-' not in nombre_de_pages :
    derniere_page = int(nombre_de_pages)+1
    premiere_page = 1
    nombre_de_pages = range(premiere_page, derniere_page)
else :
    premiere_page, derniere_page = map(int, nombre_de_pages.split('-'))
    nombre_de_pages = range(premiere_page, derniere_page+1)
    print('nombre de pages : ',nombre_de_pages)

for num_de_page_PDF in nombre_de_pages :
    nom_fichier_photo = f'{sujet_nom}_#{numero_part}_page_{num_de_page_PDF:03}.png'
        ...
        if prochain_pixel[0] == current_coords[0] :
            ...
            for x in rayon_de_vérif :
                if not x == current_x-1 :
                    for y in range(current_y, current_y+45) :
                        blue = image[y,x,0]
                        red = image[y,x,2]
            ...
            if filtered_top_vérif == [] :
                image[current_coords[0]-30:current_coords[0]+30, current_coords[1]-2:current_coords[1]+2] = [0, 255, 0]
                image[current_coords[0]-2:current_coords[0]+2, current_coords[1]-30:current_coords[1]+30] = [0, 255, 0]
